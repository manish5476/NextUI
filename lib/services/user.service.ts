...
export class UserService extends BaseApiService {
  ...
  static async getAllUsers(): Promise<ApiResponse<PaginatedResponse<User>>> {
    const service = new UserService()
    return service.get<ApiResponse<PaginatedResponse<User>>>("/v1/users/allusers")
  }

  // Update user password
  static async updatePassword(data: { currentPassword: string; newPassword: string }): Promise<ApiResponse<void>> {
    const service = new UserService()
